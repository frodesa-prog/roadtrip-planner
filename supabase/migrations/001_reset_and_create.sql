-- =============================================
-- Roadtrip Planner – Reset & Create Schema
-- Trygg å kjøre selv om tabeller allerede finnes
-- =============================================

-- Rydd opp eksisterende tabeller (i riktig rekkefølge pga. foreign keys)
...
create policy "Tilgang via tur" on stops for all
  using (exists (
    select 1 from trips where trips.id = stops.trip_id and trips.owner_id = auth.uid()
  ));
...
create policy "Tilgang via tur" on photos for all
  using (exists (
    select 1 from trips where trips.id = photos.trip_id and trips.owner_id = auth.uid()
  ));

-- Budget items
create policy "Tilgang via tur" on budget_items for all
  using (exists (
    select 1 from trips where trips.id = budget_items.trip_id and trips.owner_id = auth.uid()
  ));
