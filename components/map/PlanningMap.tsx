          ...
          {(() => {
            ...
            return (
              <button
                onClick={() => {
                  const next = !allOn
                  ;(['stops', 'activities', 'possible', 'dining', 'route'] as LayerKey[]).forEach((k) => {
                    if (layerVisibility[k] !== next) onLayerToggle(k)
                  })
                }}
                className="w-full flex items-center gap-2.5 px-3 py-2 text-[11px] font-semibold text-slate-200 hover:bg-slate-800 transition-colors border-b border-slate-700"
              >
                <span className={`w-3 h-3 rounded border flex-shrink-0 flex items-center justify-center ${
                  allOn ? 'border-blue-400 bg-blue-500' : 'border-slate-600 bg-transparent'
                }`}>
                  {allOn && <Check className="w-2 h-2 text-white" strokeWidth={3} />}
                </span>
