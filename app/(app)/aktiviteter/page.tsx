...
export default function AktiviteterPage() {
  ...
  function handleSelectActivity(id: string) {
    ...
    setSelectedActivityId((prev) => {
      if (prev === id) { setRouteInfo(null); return null }
      setRouteInfo(null)
      return id
    })
    setTimeout(() => {
  ...
  function handleSelectDining(id: string) {
    ...
    setSelectedDiningId((prev) => {
      if (prev === id) { setDiningRouteInfo(null); return null }
      setDiningRouteInfo(null)
      return id
    })
    setTimeout(() => {
  ...
  function handleDeleteActivity(id: string, name: string) {
    setConfirm({
      message: `Fjerne aktiviteten «${name}»?`,
      action: () => {
        if (selectedActivityId === id) { setSelectedActivityId(null); setRouteInfo(null) }
        removeActivity(id)
      },
    })
  }

  function handleDeleteDining(id: string, name: string) {
    setConfirm({
      message: `Fjerne spisestedet «${name}»?`,
      action: () => {
        if (selectedDiningId === id) { setSelectedDiningId(null); setDiningRouteInfo(null) }
        removeDining(id)
      },
    })
  }

  function handleRemoveActivityPin(id: string) {
    updateActivity(id, { map_lat: null, map_lng: null })
    if (selectedActivityId === id) setRouteInfo(null)
  }

  function handleRemoveDiningPin(id: string) {
    updateDining(id, { map_lat: null, map_lng: null })
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {hasPinnedLocation && (
                            <button
                              onClick={() => handleRemoveActivityPin(activity.id)}
                              title="Fjern kartpin"
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {hasPinnedLocation && (
                            <button
                              onClick={() => handleRemoveDiningPin(d.id)}
                              title="Fjern kartpin"
                              className="p-1.5 rounded text-slate-500 hover:text-amber-400 transition-colors"
                            >
                              <MapPinOff className="w-3.5 h-3.5" />
                          ...
                          <button
                            onClick={() => handleDeleteDining(d.id, d.name)}
                            title="Slett spisested"
                            className="p-1.5 rounded text-slate-500 hover:text-red-400 transition-colors"
                          >
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
          ...
          {selectedActivity && (
            <div className="absolute bottom-5 left-4 z-20 bg-slate-900/95 backdrop-blur-sm border border-slate-700 rounded-2xl shadow-2xl p-4 w-72 max-w-[calc(100%-2rem)]">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-slate-100 leading-tight">{selectedActivity.name}</p>
                  {selectedStop && (
                ...
                <button
                  onClick={() => { setSelectedActivityId(null); setRouteInfo(null) }}
              ...
              <div className="mt-3 space-y-1.5">
                ...
                {activityRoute && (
                  <div className="flex items-center gap-2 text-xs text-amber-400">
                    ...
                    <span>
                      Rute til{' '}
                      {selectedHotel?.name ? selectedHotel.name : selectedStop?.city ?? 'hotellet'}
                    </span>
                  </div>
                )}
                {routeInfo && (
              ...
              <div className="mt-3 flex gap-2 flex-wrap">
                {selectedActivity.url && (
                  <a
                    ...
                    className="flex-1 flex items-center justify-center gap-1.5 h-8 rounded-lg bg-blue-700/80 hover:bg-blue-600 text-white text-xs font-medium transition-colors"
                  >
                ...
                {selectedActivity.map_lat && selectedActivity.map_lng && (
                  <button
                    onClick={() => handleRemoveActivityPin(selectedActivity.id)}
          ...
          {selectedDining && (
            <div className="absolute bottom-5 left-4 z-20 bg-slate-900/95 backdrop-blur-sm border border-red-800/50 rounded-2xl shadow-2xl p-4 w-72 max-w-[calc(100%-2rem)]">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-slate-100 leading-tight">{selectedDining.name}</p>
                  {selectedDiningStop && (
                    <p className="text-[11px] text-slate-500 mt-0.5">
              ...
              <div className="mt-3 space-y-1.5">
                ...
                {diningRoute && (
                  <div className="flex items-center gap-2 text-xs text-red-400">
                    ...
                    <span>
                      Rute til{' '}
                      {selectedDiningHotel?.name ? selectedDiningHotel.name : selectedDiningStop?.city ?? 'hotellet'}
                    </span>
                  </div>
                )}
                {diningRouteInfo && (
              ...
              <div className="mt-3 flex gap-2 flex-wrap">
                {selectedDining.url && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1 flex items-center justify-center gap-1.5 h-8 rounded-lg bg-red-700/80 hover:bg-red-600 text-white text-xs font-medium transition-colors"
                  >
                    <ExternalLink className="w-3 h-3" />
                    Åpne lenke
                  </a>
                ...
                {selectedDining.map_lat && selectedDining.map_lng && (
                  <button
                    onClick={() => handleRemoveDiningPin(selectedDining.id)}
                    className="h-8 px-2.5 rounded-lg bg-slate-800 hover:bg-amber-900/40 text-slate-400 hover:text-amber-300 text-xs transition-colors flex items-center gap-1"
                  >
                    <MapPinOff className="w-3.5 h-3.5" />
                    Fjern pin
